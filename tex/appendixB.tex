...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \beta_1}$} (S1iotabeta);
    \draw[-{Latex[length=0.2cm]},blue] (S1) -- node[above left] {$R^{\iota,
    \beta}$} (S1iotabeta);
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \node[right] at (T) {$T$};
    \draw[-{Latex[length=0.2cm]}] (S) -- node[left] {$\bar{R'}_{<k}$} (Spk1);
    ...
    %\draw[-{Latex[length=0.3cm]}] (c1) -- node[above] {$f_1$} (c3);
  \end{tikzpicture}
  \caption{We transform $\bar{R'}$ by moving $R^{\iota, \beta}$ to the beginning
  of the sequence. The halting state is the same because of
  Lemma~\ref{lem:lemma2}. The new sequence $R^{\iota,\beta}, \bar{R'}_{<k},
  \bar{R'}_{>k}$ begins with the same step as $\bar{R}$. Thus we can examine the
  two shorter sequences $\bar{R'}_{<k}, \bar{R'}_{>k}$ and $\bar{R}_{\geq 2}$
  and conclude that $T \simeq T'$ by the induction hypothesis.}
  \label{fig:thm_qd_pf}
\end{figure}
