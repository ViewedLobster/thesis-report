...
\begin{def:ocapreach}
  For a heap $H$ and local environment $L$ we have $\ocr{(L,H)}$ iff
  \begin{align}
    \label{eq:ocr_def}
    \forall (x &\mapsto k) \in L, o \in \dom(H). \notag\\
    &\reach{(H, k)} \implies \ocap{(\typeOf{(o, H)})}
  \end{align}
\end{def:ocapreach}
...
\begin{figure}
  \scax{OCR-Empty}
  {H; a \vdash \varepsilon \: \ocr }

  \RuleSpace{}

  \scrule{OCR-FS}
  { H; a \vdash F \: \ocr \andalso H; a \vdash FS \: \ocr }
  { H; a \vdash F \circ FS \: \ocr }

  \RuleSpace{}

  \scrule{OCR-F}
  { a = \ocap \implies \ocr(L, H) }
  { H; a \vdash \sFrame{L}{t} \: \ocr }

  \RuleSpace{}

  \scax{OCR-PEmpty}
  {H \vdash \emptyset \: \ocr}

  \RuleSpace{}

  \scrule{OCR-P}
  {H \vdash P \: \ocr \andalso H; a \vdash FS \: \ocr}
  {H \vdash P \cup \left\{ FS|_a^d \right\}}
