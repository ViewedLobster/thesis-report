...
\begin{definition}{(Heap Objects)}
  ...
  form
  \begin{equation*}
    (l' , (L_{\text{env}}, z \Rightarrow t))^d
  \end{equation*}
  where $l' \in \LatVals$, $L_{\text{env}}$ is a local environment, $z \in
  \VarNames$, $t$ is a term and $d \in \TIDs$ is a unique thread identifier. The
  uniqueness of $d$ is global to the state of a program.
\end{definition}
...
\begin{definition}{(Thread Sets)}
  ...
  all of these as either the empty stack $FS = \varepsilon$, or as $FS = F \circ
  GS$ where $GS$ is also a frame stack.

  A \emph{thread set} $P$ is a finite set 
  \begin{equation*}
    P = \left\{ FS_i|_{a_i}^{d_i} \right\}_{i = 1}^{n}
  \end{equation*}
  of frame stacks $FS_i$ tagged with a unique thread id $d_i \in \TIDs$ and an
...
\begin{figure}[]
  ...
  \scrule{E-NewCell}
  {o\text{ fresh object reference } \andalso
...
In figures~\ref{fig:frame_red_rules} and \ref{fig:error_red_rules} all reduction
rules for single frames are defined. 
...
\begin{figure}
  ...
  {H, \sFrame{L}{ \Let{x}{ \Call{y}{m}{z} }{t} } \circ FS |_a^d\; \FSRedTo \\
  H, \Frame{L'}{t'}{x} \circ \sFrame{L}{t} \circ FS |_a^d}

  \RuleSpace{}

  \scax{E-Ret}
  {H, \Frame{L'}{x}{y} \circ \sframe{L, t} \circ FS |_a^d \; \FSRedTo \\
  H, \sFrame{L[y \mapsto L'(x)]}{t} \circ FS |_a^d }
...
\begin{figure}
  \scrule{GSep-Threads}
  {\forall FS|_a^d \in P. \: a = \ocap \implies \forall o \in \accRoots{(FS, H)}. \: \csep{(H, o, o_g)} }
  {H \vdash P \tsep \gsep}
