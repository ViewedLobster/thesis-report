...
\begin{figure}
  ...
  \andalso cb = (L_{\text{env}}, z \Rightarrow t') \\
...
\begin{def:ocapreach}
  For a heap $H$ and local environment $L$ we have $\ocrloc{(L,H)}$ iff
  \begin{align}
    \label{eq:ocr_def}
    \forall (x &\mapsto k) \in L, o \in \dom(H). \notag\\
    &\reach{(H, k, o)} \implies \ocap{(\typeOf{(o, H)})}
  \end{align}
\end{def:ocapreach}
...
\begin{def:state}
  ...
  and $P$ is a set of threads. For a state $S$ we have the relation
  \begin{equation}
    \vdash S \tsep \stateok.
  \end{equation}

  $\vdash S \tsep \stateok$ if $S = \Error$ or $S = H, P$ and
  \begin{align}
    H \vdash P \andalso H \vdash P \tsep \ocr \andalso \isolation{(H, P)}
...
\begin{figure}
  \scax{OCR-EmptyFS}
  {H; a \vdash \varepsilon \tsep \ocr }

  \RuleSpace{}

  \scrule{OCR-FS}
  { H; a \vdash F \tsep \ocr \andalso H; a \vdash FS \tsep \ocr }
  { H; a \vdash F \circ FS \tsep \ocr }

  \RuleSpace{}

  \scrule{OCR-F}
  { a = \ocap \implies \ocrloc(L, H) }
  { H; a \vdash \sFrame{L}{t} \tsep \ocr }
