...
also standard, with one thing to note being that all methods takes exactly one
...
set}. The heap is represented by a partial map from object identifiers $\OIDs$
to objects $\Objects$. The thread set is a set of threads, each of which
...
We first make a few definitions, building up to the final definition of a RACL
state.
...
saying $P$ is well typed under heap $H$. This relation is defined in
