...
Looking at Figure~\ref{fig:racl_grammar} we can see that a program $p$ consists
...
\begin{definition}[Basic Definitions] We define the following: 
  \begin{itemize}
...
In order to define the typing rules, we need two auxilliary shorthands for the
...
\begin{figure}
  ...
  \scrule{OCAP-Class}
  { 
  p = \overline{cd}~\overline{vd}~t \\
  \ClassDef{C}{D}{\overline{fd}}{\overline{md}} \in \overline{cd} \\
  \ocap{(D)} \andalso C \vdash_{\ocap} \overline{md} \\
  \forall (\VarDecl{f}{\sigma}) \in \overline{fd}. \: ocap(\sigma)
  }
  {\ocap{(C)}}
...
\begin{figure}
  ...
  \scrule{Reach2}
  {
    ...
    o'' \in \codom{(\FM)} \andalso \reach{(H, o'', o')}
  }
  { \reach{(H, o, o')} }
