...
\begin{definition}
  The set of object references is denoted $\ObjRefs$. The set of lattice values
  is denoted $\LatVals$. 
  The set $\Values$ is the set of all valid runtime values 
  \begin{equation*}
    \Values = \LatVals \cup \ObjRefs \cup \left\{ \NullVal \right\}
  \end{equation*}
\end{definition}
...
\begin{definition}
  The partial function $\default: \ValidTypes \to \Values$ is defined as follows
  \begin{equation*}
    \default(\tau) =
    \begin{cases}
...
\begin{proof} 
  ...
  \begin{description}
    \item[Case {\sc E-FSProp}:] According to this rule $P = Q \cup_D \left\{
      ...
      \begin{description}
        \item[Case {\sc E-FProp}:] By the rule definition $FS|_a^d = F \circ
          ...
          \begin{description}
            ...
            \item[Case {\sc E-Assign}:] The {\sc E-Assign} rule gives us that
              ...
              to the above proof of $H';b \vdash HS \tsep \ocr$ using
            ...
            \item[Case {\sc E-New}:] According to rule {\sc E-New}
              ...
              % TODO change all cases to include the subtype part of T-FS1 and
              % T-FS2
        ...
        \item[Case {\sc E-Call}:] From rule {\sc E-Call}
          % TODO fix E-Call definition error in L' def
          % TODO fix E-Ret definition weirdness
          ...
          % TODO sttate and prove proposition about ocr iff ocr of P-tilde
          We now prove isolation, OCAP reachability and global object
          separation. Instead of working with $P$ directly we instead use
          $\tilde{P}$. It is clear from propositions~\ref{prop:2.8} and % FIXME 
          ...
          To show this we start by using propositions~\ref{prop:2.8} and %FIXME 
          to get
          \begin{equation}
            H \vdash \tilde{P} \andalso \isolation(H, \tilde{P})
          \end{equation}
          ...
          \tilde{P'})$.
          % TODO redefine ocr rule for thread sets to be a forall rule
          % instead
    ...
    \item[Case {\sc E-Spawn}:] By rule {\sc E-Spawn} we have
      ...
      % TODO state proposition that reachable sets are the same as long as the
      % graph is the same
      % TODO state proposition that stack is ocr iff all reachables are ocap
      % TODO change ocr-p to refer to all threads in set instead of recursive
      Moving on to proving $\ocr$ we note proposition %FIXME
      which implies
      \begin{equation}
        H' \vdash P \tsep \ocr
      \end{equation}
      by proposition %FIXME
