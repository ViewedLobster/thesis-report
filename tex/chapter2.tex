...
In this final background section the object capability (OCAP) model will be
introduced.
...
object in a object oriented language could be modeled as an
instance.
...
to a method call. That is, we specify what the object should do (specify the
...
\begin{enumerate}
  ...
  \item Introduction: If instance A sends a message to (calls a method on)
    ...
    parameters).
\end{enumerate}
We can model references and objects using a directed graph.  The above four
rules imply that two disjoint components in a reference graph can never become
connected~\parencite{Miller06b}.

A language implementing the OCAP model is called an OCAP language. Most common
object oriented languages are not OCAP. This is because they allow global
